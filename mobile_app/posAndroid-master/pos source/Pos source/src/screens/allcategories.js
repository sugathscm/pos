...
class AllCategories extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        console.log("aaaaaaaaaaaaaa", this.state.allCategories, "ggggggggggggggggggggggggggg")
        if (!nextProps.userData.allCategories) {
            this.setState({
                ...this.state,
                isVisible: false
            });
        ...
        Object.keys(nextProps.userData.allCategories).forEach(function (key, index) {
            let obj = nextProps.userData.allCategories[key];
            obj._id = key;

            //setting client details
            let categoryId = obj._id;
            obj.categoryDetails = nextProps.userData.allCategories[categoryId];
            this.state.allCategories[this.state.allCategories.length] = obj;
            this.state.selectedCategoriesToDisplay[this.state.selectedCategoriesToDisplay.length] = obj;
        }.bind(this));
    ...
    deleteCategory(data) {
        ...
        Alert.alert(
            ...
            `${signUpStrings.youAreAboutToDeletethisCategory}`,
            [
                ...
                },
                {
                    text: 'Delete', onPress: () => {
                        ...
                        axios
                            .post(AppURLS.ApiBaseUrl + 'categories/categoryDelete',Obj)
                            .then(response => {
    ...
    updateSearch = search => {
        ...
        if (this.state.allCategories) {
            this.state.selectedCategoriesToDisplay = this.state.allCategories.filter((res, index) => {
            ...
            <Container>
                ...
                {this.state.searchSelected ?
                    ...
                    <Header>
                        ...
                        <Body style={{ flex: 1 }}>
                            <Title>{signUpStrings.allCategories}</Title>
                        </Body>
                        <Right>
                ...
                {
                    this.state.selectedCategoriesToDisplay ? (
                        this.state.selectedCategoriesToDisplay.length !== 0 ? (
                            this.state.selectedCategoriesToDisplay.map((res, index) => (
                                <List>
                                    <ListItem thumbnail  onLongPress={()=> this.deleteCategory(res)} onPress={()=>{
                                        this.props.navigation.navigate('EditCategory' , { data : res})
                                    }}>
                                        <Left>
                                            {res.categoryDetails.colour == 'Gray' ? <Thumbnail style={{ backgroundColor: 'gray' }}></Thumbnail> : null}
                                            {res.categoryDetails.colour == 'Red' ? <Thumbnail style={{ backgroundColor: 'red' }}></Thumbnail> : null}
                                            {res.categoryDetails.colour == 'Pink' ? <Thumbnail style={{ backgroundColor: 'pink' }}></Thumbnail> : null}
                                            {res.categoryDetails.colour == 'Orange' ? <Thumbnail style={{ backgroundColor: 'orange' }}></Thumbnail> : null}
                                            {res.categoryDetails.colour == 'Yellow' ? <Thumbnail style={{ backgroundColor: 'yellow' }}></Thumbnail> : null}
                                            {res.categoryDetails.colour == 'Green' ? <Thumbnail style={{ backgroundColor: 'green' }}></Thumbnail> : null}
                                            {res.categoryDetails.colour == 'Blue' ? <Thumbnail style={{ backgroundColor: 'blue' }}></Thumbnail> : null}
                                            {res.categoryDetails.colour == 'Purple' ? <Thumbnail style={{ backgroundColor: 'purple' }}></Thumbnail> : null}
                                        </Left>
                                        <Body>
                                            <Text>{res.categoryDetails ? res.categoryDetails.name : ""}</Text>
                                            <Text>{res.categoryDetails ? res.categoryDetails.items.length : "0"} {signUpStrings.itemsInAllCategories}</Text>
                                        </Body>
                                    </ListItem>
...
export default connect(mapStateToProps, matchDispatchToProps)(AllCategories);
