...
class AssignItemsEditScreen extends React.Component {
    ...
    componentDidMount() {
        this.setState({
            ...this.state,
            pageLoading: false,
            assignedItems: this.state.data.items

        },()=>{
            let count=0;
            let trueObj = {}
            this.state.data.items.map((item , index)=>{
                trueObj[item.itemId]=true;
            })
            console.log("TrueObj", trueObj)
            this.state.checkboxValues = trueObj
        });
        console.log("Dataaaaa" , this.state.data);
        console.log("Dataaaaa Array" , this.state.assignedItems);
    ...
    submitFormNavigateItems() {
        ...
        let category = {
            categoryId:this.state.data._id,
        ...
        axios
            .post(AppURLS.ApiBaseUrl + 'categories/categoryUpdate', category)
            .then(response => {
                            ...
                            this.state.selectedItems ? (
                                this.state.selectedItems.length !== 0 ? (
                                    this.state.selectedItems.map((res, index) => {
                                        console.log("Item Obj" , res.itemDetails._id)
                                        const uniqueKey = res.itemDetails._id; 
                                        return(
                                        <List key={index}>
                                            <ListItem thumbnail>
                                                <Left>
                                                ...
                                                <Right>
                                                        <CheckBox 
                                                            ...
                                                            onPress={() => {
                                                                ...
                                                                console.log("True Obj" , this.state.checkboxValues)
                                                            }}
...
export default connect(mapStateToProps, matchDispatchToProps)(AssignItemsEditScreen);
